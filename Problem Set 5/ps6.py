...
class Message(object):
    ...
    def build_shift_dict(self, shift):
        ...
        # add mapping for uppercase letters
        for x in range(len(upper)):
            shift_dict[upper[x]] = upper[(x + shift) % len(upper)]
        # add mapping for lowercase letters
        for x in range(len(lower)):
            shift_dict[lower[x]] = lower[(x + shift) % len(lower)]
        return shift_dict
...
class CiphertextMessage(Message):
    ...
    def decrypt_message(self):
        ...
        '''
        all_possible_shifts = 26
        best_shift = None
        num_real_words = 0
        
        for s in range(all_possible_shifts):
